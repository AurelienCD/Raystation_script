...
for elm in IDList:
    ...
    ### To select the good case ###
    for elm in patient.Cases:
        if elm.CaseName == "CFB \xe9tude complexit\xe9": 
            case = elm
        elif elm.CaseName == "CFB Etude complexit\xe9":
            case = elm
        else:
            print("there is no case named CFB Etude complexité")

    ### Set case and plan variable ###
    case.SetCurrent()
    case.TreatmentPlans[0].SetCurrent()
    plan = get_current('Plan')

    #case = patient.Cases
    #plan = case[0].TreatmentPlans[0]
